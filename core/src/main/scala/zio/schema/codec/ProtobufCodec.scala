    ...
    private def nestedFields[A, B](
              ...
              (
                ...
                (
                  ...
                  Schema.Transform[A, B](
                    ...
                    (b: B) => (b.asInstanceOf[Map[String, B]]).get(field).toRight("Missing value").asInstanceOf[Either[String, A]]
                  )
                )
              )
          }
      ...
      case Schema.Transform(codec, f, g) =>
        nestedFields(baseField, codec, nextFieldNumber).map(_.map {
          case (fieldNumber, fieldAndSchema) =>
            (fieldNumber, (fieldAndSchema._1, Schema.Transform[A, B](
              fieldAndSchema._2,
  ...
  object Encoder {
    ...
    def encode[A, B](fieldNumber: Option[Int], schema: Schema[A], value: A): Chunk[Byte] =
      (schema, value) match {
        ...
        case (Schema.Transform(codec, _, g: (A => Either[String, B]) ), _)                 => g(value).map(encode(fieldNumber, codec, _)).getOrElse(Chunk.empty)
  ...
  object Decoder {
              ...
              .map {
                case (field, schema) =>
                  fieldDecoder(wireType, schema).map {
                    case value: Seq[_] =>
                      val values = result.get(field).asInstanceOf[Option[Seq[_]]].map(_ ++ value).getOrElse(value).asInstanceOf[A] // needed for Dotty
    ...
    private def defaultValue[A](schema: Schema[A]): Option[A] = schema match {
      ...
      case Schema.Tuple(left, right) =>
        if (defaultValue(left).isEmpty || defaultValue(right).isEmpty) Option.empty[A] else Option[A]((left, right).asInstanceOf[A])
